%-------------------------------------------------------------------------------
%	SECTION TITLE
%-------------------------------------------------------------------------------
\cvsection{Research Interests}


%-------------------------------------------------------------------------------
%	CONTENT
%-------------------------------------------------------------------------------
\begin{cventries}

%---------------------------------------------------------
  \cventry
    {} % Role
    {Broadly interested in theoretical Condensed Matter Physics} % Event
    {} % Location
    {} % Date(s)
    {
      \begin{cvitems} % Description(s)
      	\item {\textbf{Tunneling phenomena in unconventional superconductors and in superconductors with broken time-reversal symmetry}}
      	\item {\textbf{Tunneling phenomena in Intervalley Coherent Phases and Altermagnets}}
      	\item {\textbf{Electronic transport in two-dimensional systems and the effects of Berry curvature in transport}}
      	\item {\textbf{Non-Fermi Liquids emerging due to subsystem symmetry breaking}}
      	\item {\textbf{Probes for Quantum Spin Liquids}}
%      	\item {\textbf{Spin chirality in systems with spontaneously broken time-reversal symmetry}}
      	%\item {\textbf{Role of Berry curvature in coherent phenomena}}
        %\item {\textbf{Computational methods in quantum condensed matter physics}}
        \item {\textbf{Topological phases of matter and Quantum Phase transitions}}
        %\item {\textbf{Thermo-electric transport and the effects of Berry curvature}}
        %\item {\textbf{Strongly correlated systems}}
        %\item {\textbf{Brownian motion}}
        %\item {\textbf{Thermalization of quantum systems and Many body localization}}
      \end{cvitems}
    }

%---------------------------------------------------------
\end{cventries}
